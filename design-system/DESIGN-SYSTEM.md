# Corporal Academy — Design System
...
| `corporal-academy-design-system.css` | **Drop-in.** Tokens (dark/light) + reset + foco + motion + tipografia + botões + chips/badges/status. Basta linkar. |
| `corporal-academy-design-tokens.json` | Tokens legíveis por máquina (Style Dictionary, Framer, plugins, agentes de IA). |
...
| `copy-deck.corporal-academy.json` | Deck de copy do produto (hero, notas, microcopy, demos) usado no showcase. Fonte canônica das strings. |
...
Importe `corporal-academy-design-tokens.json` e gere variáveis no seu formato (CSS vars, JS, Tailwind theme, etc.). Regra de ouro: **componentes consomem tokens, nunca hex solto.**
...
Ao aplicar este design system em um projeto, **leia `corporal-academy-design-tokens.json`** e siga as regras acima. Prompt sugerido:

> Você vai aplicar o **Corporal Academy Design System** (autor: Edegar Junior). Fonte da verdade: `corporal-academy-design-tokens.json` + `corporal-academy-design-system.css` desta pasta.
