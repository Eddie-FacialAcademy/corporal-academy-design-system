# Implementação — Corporal Academy Design System

Documentação técnica de **como cada item é implementado**, para subir em qualquer infra (HTML/CSS puro, qualquer stack via tokens, ou Framer). Tudo aqui reflete o que está no showcase publicado (`index.html`) e no pacote `design-system/`. Em caso de divergência, **o showcase + `corporal-academy-design-tokens.json` são a fonte da verdade**; o `corporal-academy-design-system.css` (drop-in) espelha esses tokens.
...
```
repo (corporal-academy-design-system)
...
└── design-system/                   # PACOTE consumível
    ...
    ├── corporal-academy-design-system.css   # DROP-IN: tokens + reset + foco + motion + tipografia + botões + chips/status
    ├── corporal-academy-design-tokens.json  # Tokens legíveis por máquina (Framer, Style Dictionary-like, IA)
    ...
    └── copy-deck.corporal-academy.json      # Strings de produto do showcase
```
...
### 2.5 Formato do `corporal-academy-design-tokens.json`
...
<html lang="pt-BR"><head>
  ...
  <link rel="stylesheet" href="corporal-academy-design-system.css">
</head><body>
...
**B) Tokens (qualquer stack)** — importe `corporal-academy-design-tokens.json` e gere CSS vars / JS / Tailwind theme. Componentes consomem token, nunca hex.
...
- **Repos:** `Eddie-FacialAcademy/corporal-academy-design-system` (irmão: `facialclass-design-system`).
...
- **URL ao vivo:** https://eddie-facialacademy.github.io/corporal-academy-design-system/
...
| Aspecto | Corporal Academy | Facial Class |
|---|---|---|
| Arquivos | `corporal-academy-design-system.css` · `corporal-academy-design-tokens.json` · `copy-deck.corporal-academy.json` | `facial-design-system.css` · `facial-design-tokens.json` · `copy-deck.facial.json` |
...
| Marca / Academy | Corporal Academy / Corporal Academy | Facial Class / Facial Academy |
