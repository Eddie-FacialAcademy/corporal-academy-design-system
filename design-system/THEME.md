# Tema claro/escuro — Corporal Academy Design System
...
## 1. Web (HTML/CSS) — já implementado em `corporal-academy-design-system.css`

Três camadas, nesta ordem:

```css
/* 1) Base = dark (padrão) */
:root{ --bg:#0E0708; --txt:#FAF7F8; /* … todos os tokens dark … */ color-scheme:dark; }

/* 2) Segue o sistema: se o SO está em light e o usuário NÃO escolheu manualmente */
@media (prefers-color-scheme: light){
  :root:not([data-theme="dark"]){ --bg:#FAFAFA; --txt:#2A1517; /* … light … */ color-scheme:light; }
}

/* 3) Escolha manual do usuário (toggle) vence o sistema */
[data-theme="light"]{ --bg:#FAFAFA; --txt:#2A1517; /* … light … */ color-scheme:light; }
...
```html
<script>(function(){try{var t=localStorage.getItem('cc-theme');
...
btn.addEventListener('click',function(){
  ...
  localStorage.setItem('cc-theme',n);
});
```

> `color-scheme` em cada tema faz scrollbars/controles nativos acompanharem. No light, dourado/rosa como **texto** usam as variantes `-ink`. A marca (`--logo`) é **branca** no dark e **bordô** no light.

---

## 2. Framer — como deve ser feito

1. **Color Styles com Light + Dark** (em `Corporal Academy/…`): cada estilo tem valor de Light e de Dark.
...
4. **Toggle manual no Framer (opcional):** crie uma Variable de tema (ou um Component com variantes Light/Dark) ligada a um botão; ela sobrepõe o tema do sistema, equivalente ao `data-theme` da web.
...
- [ ] Toggle opcional persiste a escolha (`cc-theme`) e sobrepõe o sistema.
